...
SELECT
  ...
  CASE
    ...
    AND capitams.capitams_key IS NULL THEN NULL
    WHEN vies.vies_dor_opinion = 'VIES ready for capitalisation'
  ...
  ---------------------------------------------------------
  -- 6. KPI 3C BASE MEASURES
  ---------------------------------------------------------

  -- 1. VIES Ready for capit
  (
    vies.vies_dor_opinion = 'VIES ready for capitalisation'
    AND vies.vies_status IN ('Ready for Deployment', 'Deploying', 'Closed')
  ) AS tickets_vies_ready,

  -- 2. CapitAMS Created
  (
    vies.vies_dor_opinion = 'VIES ready for capitalisation'
    AND vies.vies_status IN ('Ready for Deployment', 'Deploying', 'Closed')
    AND capitams.capitams_key IS NOT NULL
  ) AS tickets_capitams_created,

  -- 3. CapitAMS with NRL or NPK
  (
    vies.vies_dor_opinion = 'VIES ready for capitalisation'
    AND vies.vies_status IN ('Ready for Deployment', 'Deploying', 'Closed')
    AND (
      nrl.date_statut_valide IS NOT NULL
      OR CONTAINS_SUBSTR(IFNULL(capitams.capitams_capitalization_status, ''), 'NPK')
    )
  ) AS tickets_nrl_npk
